  ...
  document.addEventListener('keydown', e => {
    ...
    if (!inInput) {
      if (e.key === 'ArrowUp' && state.currentIndex > 0) {
        ...
        state.selectPatch(i);
        selectPatchOnDevice(i);
        // Atualiza a tela com o que a G1on carregou
        if (state.midi.connected) setTimeout(() => requestCurrentPatch(), 50);
      }
...
function _cmdSave() {
  ...
  // 1. Salva no LittleFS local
  savePatch({ ...patch, slot: idx });
  
  // 2. Salva na pedaleira física
...
function _exposeUI() {
  window._ui = {
    ...
    selectPatch: (i) => { 
      ...
      // Atualiza a tela com o que a G1on carregou
      if (state.midi.connected) {
        setTimeout(() => requestCurrentPatch(), 50);
      }
    },
    ...
    // Hook exposto para midi_manager e param_editor
    onPatchReceivedForSync,
    setInteracting:     (val) => state.setInteracting(val)
  };
}