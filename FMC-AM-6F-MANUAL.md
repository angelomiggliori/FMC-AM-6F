...
5. [Banco de Efeitos (ZDL JSON Nativo)](#5-banco-de-efeitos-zdl-json-nativo)
6. [Sistema de Dados e Backup](#6-sistema-de-dados-e-backup)
7. [Tap Tempo e Signal Chain Editor](#7-tap-tempo-e-signal-chain-editor)
...
12. [Como Contribuir / Continuar](#12-como-contribuir--continuar)
13. [Histórico de Descobertas ZDL (Motor 4.2 Modular)](#13-histórico-de-descobertas-zdl-motor-4-2-modular)
...
- Ver o nome do patch e Volume na tela.
- Signal Chain: Visualizar os blocos acionados, desativá-los ou fazer edições de atributos em tempo real!
- Tap Tempo nativamente sincronizado à Engine Interna da pedaleira.
- Tuner via SysEx
- Boost de volume por patch
- Sistema híbrido (LocalDB / GitHub) de persistência
- 5 temas visuais distintos (HeadRush, Boss, Kemper, Zoom nativo)

**Stack técnica:**
- HTML + CSS + JavaScript nativo ES6 Modules
- Web MIDI API nativa (Chrome/Edge)
- Repositório Zlib Revertido: Dicionários reais compilados a partir de `.ELF` DSP nativos usando ferramentas em C# e PowerShell.
...
Na Versão **4.2**, abandonamos o monolito de Javascript. Todo o projeto agora roda estritamente estruturado sob Módulos Independentes:

```
fmc-am-6f/
│
├── engine/                ← MOTOR MODULAR
│   ├── main.js            ← Boot e inicialização
│   ├── midi-core.js       ← WebMIDI API, Handshake, SysEx queue
│   ├── patch-manager.js   ← Parsing de Dumps 7-bit, Cache, PCs
│   ├── tap-engine.js      ← Lógica matemática de BPM e mapeamento
│   ├── signal-chain.js    ← Eventos do UI, Double-tap pra janela de edição
│   ├── gui-adapter.js     ← Binding responsivo do HTML para o JS engine
│   ├── footswitch.js      ← Controla 12 botões, holding
│   └── tuner/boost.js     ← Features extras
│
├── data/                  ← DADOS EXTRAÍDOS
│   ├── data-manager.js    ← Sistema LocalDB e GithubDB
│   ├── fx-db.json         ← Referência Hex de IDs ToneLib
│   └── fx-params.json     ← Catálogo Definitivo de strings do hardware (Descoberta ZDL)
│
├── ZDL/                   ← SCRIPTS DE COMPILAÇÃO
│   ├── dump.ps1 e scripts ← Quebra QRES/Zlib pra C# e PowerShell
│   └── all_effects_table.md ← Tabela documentada gerada pela engine reversa
│
├── index.html             ← Hub de navegação
├── theme-headrush.html    ← Tema Headrush Principal (Atualizado V4.2 com UI Grid)
└── temas_antigos...       ← Temas legados
```

### Como os temas carregam o engine

Cada HTML de tema inclui apenas:
`<script type="module" src="engine/main.js"></script>`
E define constantes visuais em CSS e IDs que a classe `gui-adapter.js` lê independentemente.
...
## 5. Banco de Efeitos (ZDL JSON Nativo)

### Fim do `ZOOM_FX_DB` Manual
Até a versão 4.1, precisávamos adivinhar quais efeitos o Zoom possuía usando chaves fixadas (`const ZOOM_FX_DB`). **Isso não é mais necessário**.
O Motor 4.2 realizou engenharia reversa diretamente no contêiner DSP `effects_zdl.dat`. Mais de **900 blocos binários (ELF)** foram descompactados usando *DeflateStreams* em C#. 

Com um script PowerShell, o motor escaneou as tabelas de símbolos (strings lógicas), rastreando a ancora `"OnOff"`, lendo a verdadeira nomenclatura do Firmware e empacotando numa fusão entre `idHex` do aplicativo *ToneLib* da fabricante com as chaves extraídas.

O resultado é consumido vivo em `data/fx-params.json` e `data/fx-db.json`.

```json
  "0x280B": {
      "params": ["Time", "HiDMP", "Level", "Tail", "MONO", "divu.asm"],
      "tapParamIdx": 1,
      "tap": "Time"
  }
```
**Campos Decifrados:**
- `params`: Nomes exatos do *knobs* de hardware para a UI mostrar sem depender do App.
- `tap`: Se o efeito ouve ou não o footswitch do Tempo (`Time` ou `Rate`).
- `tapParamIdx`: Qual index real numérico injetar o `CMD_PARAM` para calibrar a variável musical. Nenhuma adivinhação.
...
## 7. Tap Tempo e Signal Chain Editor

### 7.1 Como funciona o Tap

1. Usuário toca FS6/FS11 (tap) em ritmo
2. `tap-engine.js` calcula a média isolando erros, restritamente entre 40~250 BPM.
3. Consulta no slot de cada efeito ligado do patch as regras de index de `fx-params.json`.
4. Converte os milissegundos e envia exatamente para o índice (`tapParamIdx`) do componente.

### 7.2 Edição Plena Visual (Signal Chain)
O Signal Chain substitui o *ToneWebLib* visualmente direto no `theme-headrush.html`.
- **Toque único**: Desliga ou Religa o pedal sem mudar presets (`CMD_PARAM` flag `0x80`).
- **Toque duplo**: Invoca o Modulator `editorOverlay`.

O Modulator lê os parâmetros disponíveis no nosso arquivo *ZDL* providenciando **Sliders**. Mexer no slider remonta a regra técnica e joga a requisição crua `CMD_PARAM(31) 14hbits` que controla os milésimos da pedaleira. Não mostramos numerais absolutos de fábrica exatos, pois a Zoom utiliza criptografia espalhada (*Scrambled MS-50G Style*), tornando a visualização imperfeita, mas a escrita soberana.
...
## 13. Histórico de Descobertas ZDL (Motor 4.2 Modular)

### A Arquitetura ZDL Descompactada (16/03/2026)
- **Container Binário:** Evidenciado que arquivos de atualização tipo `.dat` não eram blocos criptografados, mas contêineres tipo **QRES** agrupando arquivos Zlib (`78 DA`).
- **Módulos ELF:** Confirmamos visualmente pelo dump que cada efeito é compilado para o próprio processador no formato ELF de 32-bit (Executable and Linkable Format).
- **Índices Nativos:** Substituiu-se dezenas de chutes matemáticos pela verdadeira ordem de Arrays presente dentro do binário `ZDL` original (`OnOff`, `Time`, `Rate`, `Level`, `Tail`). 
- **Scrambled Bytes:** Confirmou a documentação de repositórios open-source onde o *Display* natural visual precisaria de uma biblioteca dicionária extensa por não seguir uma norma uníssona em hexadecimal nas saídas de `SysEx (28h)`. Resolvem-se escrevendo por `CMD_PARAM (31h)`.

---

*Última Documentação Automática v4.2 Modular | AI System | 18/03/2026*
