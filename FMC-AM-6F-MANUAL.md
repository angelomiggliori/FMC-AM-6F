...
## 18. Descobertas de 2026-03-18 — Tap Tempo via Write 0x28

### Write 0x28 bidirecional — CONFIRMADO

O mesmo comando `0x28` usado pela G1On para enviar dumps é aceito bidirecionalmente para **receber** um patch completo. Enviar 134 bytes com `F0 52 00 63 28 [payload] F7` pelo host atualiza o patch ativo na RAM da G1On instantaneamente, sem PC, sem reload.

### CMD_PARAM não controla Time — CONFIRMADO

O Time do Delay está codificado no próprio ID do efeito (`rawSlot[4]` = id_lo). Varredura completa de `paramIdx=0` a `11` confirmou que nenhum paramIdx altera o id_lo via CMD_PARAM `0x31`. CMD_PARAM altera outros bytes do slot (parâmetros como Feedback, Mix, etc.) mas não o Time.

### Fluxo do Tap Tempo (novo)

```
1. Recebe dump → guarda 134 bytes em tapRawDump
2. Usuário pisa TAP → calcula BPM → ms
3. Para cada efeito com tap:'Time':
   a. Interpola id na tabela de calibração
   b. Modifica rawSlot[3] (id_hi) e rawSlot[4] (id_lo)
   c. Repacota com pack7bit()
   d. Envia 134 bytes via F0 52 00 63 28 ... F7
4. G1On aceita imediatamente
```

### Tabela de calibração — Delay 0x28xx (confirmada)
...
Interpolação linear entre pontos para valores intermediários.

### Layout de Footswitches — DEFINITIVO (modo 12)

```
[FS1 ][FS2 ][FS3 ][FS4 ][FS5 ][BANK]
[FS7 ][FS8 ][FS9 ][FS10][FS11][TAP ]

Índices:
  0-4  = patches 0-4    (FS1-FS5)
  5    = BANK            press=bank select / hold=nada
  6-10 = patches 5-9    (FS7-FS11)
  11   = TAP             press=tap tempo / hold=tuner
```
