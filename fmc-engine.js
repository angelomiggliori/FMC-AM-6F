...
// ══════════════════════════════════════════════════════════════════
const ZOOM_FX_DB = {
  // IDs confirmados por engenharia reversa dos dumps reais fw 1.21
  // id2 = (raw[0]<<7)|raw[1]  ← DOIS bytes (raw[2] = byte de MSBs dos parâmetros)
  // ── Estruturais (sem tap) ─────────────────────────────────────
  0x2100:({n:'AmpSim',   c:'amp',      t:null}),
  0x22C8:({n:'Cab-A',    c:'cab',      t:null}),
  0x0004:({n:'ZNR',      c:'dynamics', t:null}),
  0x0008:({n:'GraphicEQ',c:'filter',   t:null}),
  0x000C:({n:'Cry',      c:'filter',   t:null}),
  0x0300:({n:'AmpSim-B', c:'amp',      t:null}),
  0x0201:({n:'AmpSim-C', c:'amp',      t:null}),
  0x0600:({n:'AmpSim-D', c:'amp',      t:null}),
  0x0010:({n:'OverDrive', c:'drive',   t:null}),
  // ── Delays (Time) ─────────────────────────────────────────────
  0x0050:({n:'Delay',    c:'delay',    t:'Time'}),
...
function parseZoomDump(data){
  ...
  for(let i=0;i<5;i++){
    ...
    const id2 = (r0<<7)|r1;
    const en  = (r0&0x01)===1;
    const fx  = ZOOM_FX_DB[id2]||{n:`FX_${id2.toString(16).toUpperCase()}`,c:'unknown',t:null};
...
const TAP_PARAM_MAP = {
  ...
  // Protocolo: F0 52 00 63 31 [slotIdx 0-4] [rawIdx] [valLo] [valHi] F7
  // rawIdx = índice absoluto no rawSlot[] (não paramIdx lógico)
  //
  // ── Delays (Time) ─────────────────────────────────────────────────
  0x0050:({raw:6, tipo:'ms', escala:8.0}),     // Delay     ✅ E5  560ms→70
  0x1880:({raw:6, tipo:'ms', escala:10.0}),    // TapeEcho  ✅ E9  560ms→56
  0x280B:({raw:6, tipo:'ms', escala:8.0}),     // Delay     ✅ J0  560ms→70
  0x0990:({raw:6, tipo:'ms', escala:1.719}),   // StereoDly ✅ E9
  0x287C:({raw:6, tipo:'ms', escala:7.8125}),  // FilterDly ✅ E6
  0x286D:({raw:6, tipo:'ms', escala:46.875}),  // MultiTapD ✅ E7
  0x2816:({raw:7, tipo:'ms', escala:9.0}),     // PitchDly  ✅ E8
  // ── Modulações (Rate) ─────────────────────────────────────────────
  0x0480:({raw:6, tipo:'hz', escala:2.0}),     // Chorus    ✅ E0  rate25→val50
  0x1080:({raw:4, tipo:'hz', escala:8.0}),     // Flanger   ✅ E1  rate7→val56
  0x214C:({raw:4, tipo:'hz', escala:8.0}),     // Flanger   ✅ J0
  0x004C:({raw:3, tipo:'hz', escala:2.0}),     // Phaser    ✅ E2  rate12→val24
  0x0614:({raw:4, tipo:'hz', escala:0.25}),    // Tremolo   ✅ E4  rate33→val8
  0x0180:({raw:4, tipo:'hz', escala:0.25}),    // Tremolo   ✅ J0
  0x090C:({raw:9, tipo:'hz', escala:0.5}),     // Vibrato   ✅ E4  rate30→val15
  0x0100:({raw:9, tipo:'hz', escala:0.5}),     // Vibrato   ✅ J0
};
