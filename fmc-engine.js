...
function limparCachePatches(){
  ...
  // Força hard refresh da página para sincronizar UI com seed
  // Use true como argumento para ignorar cache HTTP do navegador
...
function parseZoomDump(data){
  ...
  // Slots — id2 = (raw[0]<<7)|raw[1], 2 bytes (confirmado fw 1.21)
  for(let i=0;i<5;i++){
    const off = 9 + i*20;
...
//
// id2 = (raw[0]<<7)|raw[1]  ← parser de 2 bytes (não 3!)
...
function enviarSysExTapTempo(bpm){
  ...
  sysexIdleTimer = setTimeout(async()=>{
    ...
    for(const efeito of efeitosAlvo){
      // id2 = (raw[0]<<7)|raw[1] — 2 bytes, já calculado corretamente no parseZoomDump
      const id2 = efeito.id;  // ← é id2, não id3
      const map = TAP_PARAM_MAP[id2];
      if(!map){
        showToast(`TAP: ${efeito.nome} ID=0x${id2.toString(16).toUpperCase()} SEM MAPA`);
        logMIDI({type:'sx', label:`TAP ${bpmCalc}bpm — ${efeito.nome}(0x${id2.toString(16).toUpperCase()}) sem mapeamento`});
        continue;
      }
