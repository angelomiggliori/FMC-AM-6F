...
//   Modo 12 FS: FS1-5=patches 0-4, FS6=BANK(press)/nada(hold), FS7-11=patches 5-9, FS12=TAP(press)/TUNER(hold)
//
...
async function fecharEditor(){
  // ⚠ CUIDADO: CMD_EDIT_OFF (0x51) grava no flash da G1On — latência 800ms+
  // Só chamar ao desconectar deliberadamente, NUNCA entre patches
  if(!midiOut||!midiReady) return;
...
for(let i=0;i<FS_COUNT;i++){
  ...
  if(FS_MODE===12){
    // Layout:
    //   idx 0-4  = FS1-FS5  (patches 0-4)
    //   idx 5    = BANK      press=bank select / hold=nada
    //   idx 6-10 = FS7-FS11 (patches 5-9)
    //   idx 11   = TAP       press=tap / hold=tuner
    if(i === 5){
      ...
      s.textContent='BANK SEL';btn.appendChild(s);
    } else if(i === 11){
      btn.appendChild(document.createTextNode('TAP'));
      const s=document.createElement('span');s.style.cssText='font-size:7px;opacity:.6;display:block;margin-top:1px';
      s.textContent='HOLD=TUNE';btn.appendChild(s);
      btn.classList.add('tap-btn');
    } else {
      // FS1-5 (idx 0-4) e FS7-11 (idx 6-10)
      const fsNum = i < 5 ? i+1 : i+1; // FS1-FS5, FS7-FS11 (skip FS6=BANK)
      btn.appendChild(document.createTextNode(`FS${fsNum}`));
    }
  } else {
...
// ── 12 FS navigation ────────────────────────────────────────────
// idx 0-4 = patches 0-4, idx 5 = BANK, idx 6-10 = patches 5-9, idx 11 = TAP
function onFSPatch12(patchIdx){
  // patchIdx já vem convertido do dispatcher (idx<5?idx:idx-1)
  if(bankSelect){
    // Bank select: patchIdx 0-4 = bancos A-E, patchIdx 5-9 = bancos F-J
    const banco = BANKS_ALL[patchIdx];
    if(!banco) return;
    if(preselectBank === banco){
      ...
      limparPatchAtual();
      enviarProgramChange(BANKS_ALL.indexOf(cam0.bank)*10 + cam0.patch);
      // fsEl idx: patchIdx 0-4 → fsEls[0-4], patchIdx 5-9 → fsEls[6-10]
      const fsI = patchIdx < 5 ? patchIdx : patchIdx + 1;
      flashConfirmRing(fsEls[fsI]);
  ...
  cam0.patch = patchIdx;
  limparPatchAtual();
  enviarProgramChange(BANKS_ALL.indexOf(cam0.bank)*10 + patchIdx);
  const fsI = patchIdx < 5 ? patchIdx : patchIdx + 1;
  flashConfirmRing(fsEls[fsI]);
  gravarEstado(); renderizar();
}
...
function onFS11Press(){
  ...
  if(bankSelect){
    ...
    showToast('BANK SELECT · A–E nos FS1–5 · F–J nos FS7–11');
  }
  renderizar();
}
...
if(FS_MODE === 12){
  ...
  renderizar = function renderizar12(){
    ...
    if(bankSelect && preselectBank){
      ...
      $patchId.textContent = '??'; $patchId.className='patch-id blink';
      $patchName.textContent = 'SELECIONE O BANCO · FS1–FS5 + FS7–FS11';
      $display.style.setProperty('--bank-color', '#ff6b00');
    } else {
    ...
    // Layout definitivo:
    //   idx 0-4  → patches 0-4   (FS1-FS5)
    //   idx 5    → BANK
    //   idx 6-10 → patches 5-9   (FS7-FS11)
    //   idx 11   → TAP

    if(bankSelect){
      // FS1-FS5 (idx 0-4) = bancos A-E
      // FS7-FS11 (idx 6-10) = bancos F-J
      // idx 5 (BANK) = cancelar
      for(let i=0; i<12; i++){
        if(i === 5)  continue; // BANK btn tratado separado
        if(i === 11) continue; // TAP sempre ativo
        const bankIdx = i < 5 ? i : i-1; // idx 0-4→0-4, idx 6-10→5-9
        const b  = BANKS_ALL[bankIdx];
        const bc = corDoBanco(b);
        const btn = fsEls[i];
      ...
      for(let i=0; i<5; i++){
        ...
        btn.style.borderColor=''; btn.style.boxShadow=''; btn.style.color='';
        btn.classList.toggle('lit-top', isActive);
        btn.childNodes[0].textContent = `FS${i+1}`;
        const cached = lerCachePatch(bank, i);
        labelEls[i].textContent = (cached?.nome||`P${i+1}`).substring(0,10);
      ...
      fsEls[5].classList.remove('lit-top','lit-bottom','preselect-mode');
      fsEls[5].style.borderColor=''; fsEls[5].style.boxShadow=''; fsEls[5].style.color='';
      labelEls[5].textContent = 'BANK SEL';
      labelEls[5].style.color=''; labelEls[5].style.textShadow='';
      // FS7-FS11 (idx 6-10) = patches 5-9
      for(let i=6; i<11; i++){
        const pIdx = i - 1; // idx 6→patch5, 7→6, 8→7, 9→8, 10→9
        const btn  = fsEls[i];
        const isActive = cam0.patch === pIdx;
        btn.style.setProperty('--bank-color', cor);
        btn.classList.remove('preselect-mode');
        btn.style.borderColor=''; btn.style.boxShadow=''; btn.style.color='';
        btn.classList.toggle('lit-top', isActive);
        btn.childNodes[0].textContent = `FS${i+1}`;
        const cached = lerCachePatch(bank, pIdx);
        labelEls[i].textContent = (cached?.nome||`P${pIdx+1}`).substring(0,10);
        labelEls[i].style.color = isActive ? cor : '';
        labelEls[i].style.textShadow = isActive ? `0 0 6px ${cor}` : '';
      }
      // TAP (idx 11)
      fsEls[11].classList.remove('preselect-mode');
      labelEls[11].textContent = tunerAtivo ? '🎸 AFINAR' : 'TAP';
